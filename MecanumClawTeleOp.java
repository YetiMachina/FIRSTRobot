...
@TeleOp(name="Mecanum TeleOp, 4W, Claw", group="Robot")
//@Disabled
public class MecanumClawTeleOp extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            double leftF = y + x + xr;
            double leftB = y - x + xr;
            double rightF = -(y - x - xr);
            double rightB = -(y + x - xr);


            /* Normalize the values so neither exceed +/- 1.0 */
            maxFront = Math.max(Math.abs(leftF), Math.abs(rightF));
            if (maxFront > 1.0)
            {
                leftF /= maxFront;
                rightF /= maxFront;
            ...
            maxBack = Math.max(Math.abs(rightB), Math.abs(leftB));
            if (maxBack > 1.0)
            {
                leftB /= maxBack;
                rightB /= maxBack;
            ...
            leftBackDrive.setPower(leftB);
            rightBackDrive.setPower(rightB);
            
            double extendoFun = -gamepad1.right_stick_y;
            
            theBitThatExtends.setPower(extendoFun);
